...
exports.registerUser=async function(req,res){
    ...
    res.status(201).cookie("token",token,{maxAge:age,secure:true}).json(createdUser)    
    }
    catch(err){
...
exports.loginUser=async function(req,res){
    try{
        ...
        res.status(200).cookie("token",token,{maxAge:age,secure:true}).json(restUserDetails)   
    }
    catch(err){
        console.log(err)
...
exports.logoutUser=function (req,res){
    res.clearCookie("token",{secure:true}).json({message:"Logout Complete"})
}